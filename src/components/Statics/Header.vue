  ...
  <div class="container-fluid __header fixed-top">
    <nav class="navbar navbar-expand-lg navbar-light">
      <div class="navbar-brand d-flex flex-row">
        ...
        <router-link tag="a" to="/post/new" class="ml-4 __post">
          <button type="button" class="btn btn__normal pl-2 pr-2" style="color: white;">Publier</button>
        </router-link>
      </div>
      ...
      <div class="collapse navbar-collapse navbar-toggleable-xs" id="navbarSupportedContent">
        <!-- <form class="form-inline mr-auto my-2 my-lg-0 order-xs-2 order-sm-2 order-md-1">
          <input class="form-control mr-sm-2 my_search" type="text" placeholder="Qu'est-ce que vous recherchez ?" aria-label="Search">
          <button class="btn btn__normal search my-2 my-sm-0" type="submit">Trouver</button>
        </form> -->
        <div class="mr-auto"></div>
        <ul class="navbar-nav navbar-right order-xs-1 order-sm-1 order-md-2 navbar__search">
          <router-link tag="a" activeClass="active" to="/post/new" class="nav-item mr-md-3 __when_post">
            <button type="button" class="nav-link btn btn__normal pl-2 pr-2" style="color: white;">Publier</button>
          </router-link>
          ...
          <li class="nav-item dropdown" v-if="userIsAuthenticated">
            <a class="nav-link dropdown-toggle d-flex align-items-center" href="http://example.com" id="navbarDropdownMenuLink" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
              <img :src="user.avatar" alt="" class="avatar mr-2" width="20px" height="20px" v-if="user.avatar"> Compte
            </a>
          ...
          <li class="nav-item" v-if="userIsAuthenticated"><a class="nav-link" @click="onLogout">Se déconnecter</a></li>
        </ul>
      </div>
    </nav>
  </div>
  ...
  computed: {
    ...
    menuDrawer () {
      ...
      if (!this.userIsAuthenticated) {
        menuItems = [
          {title: 'Se connecter', link: '/signin', icon: 'person'},
          {title: 'S\'inscrire', link: '/signup', icon: 'person_add'},
        ]
      }
      return menuItems
    }
